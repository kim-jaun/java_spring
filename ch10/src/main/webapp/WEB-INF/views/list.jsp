			...
			<c:if test="${board2.del!='y' }">
				<td title="${board2.content }">
					<c:if test="${board2.re_level > 0 }">
						...
						<img alt="" src="/ch10/resources/images/re.gif">
					</c:if>
					<a href="${path}/view/num/${board2.num}/pageNum/${pb.currentPage}?keyword=${board.keyword}&search=${board.search}"
...
<div align="center">
	...
	<!-- 시작 페이지가 pagePerBlock보다 크면 앞에 보여줄 것이 있다 -->
		<c:if test="${pb.startPage > pb.pagePerBlock }">
			<li><a href="${path}/list/pageNum/1?keyword=${board.keyword}&search=${board.search}">
				<span class="glyphicon glyphicon-backward"></span></a></li>
			<li><a href="${path}/list/pageNum/${pb.startPage-1}?keyword=${board.keyword}&search=${board.search}">
				<span class="glyphicon glyphicon-triangle-left"></span></a></li>
		</c:if>
		<c:forEach var="i" begin="${pb.startPage }" end="${pb.endPage }">
			<c:if test="${pb.currentPage==i }">
				<li class="active"><a href="${path}/list/pageNum/${i}?keyword=${board.keyword}&search=${board.search}">${i}</a></li>
			</c:if>
			<c:if test="${pb.currentPage!=i }">
				<li><a href="${path}/list/pageNum/${i}?keyword=${board.keyword}&search=${board.search}">${i}</a></li>
			</c:if>
		</c:forEach>
		<!-- 보여줄 것이 남아있는 경우에는 endPage보다 totalPage가 큰경우 -->
		<c:if test="${pb.endPage < pb.totalPage }">
			<li><a href="${path}/list/pageNum/${pb.endPage+1}?keyword=${board.keyword}&search=${board.search}">
				<span class="glyphicon glyphicon-triangle-right"></span></a></li>
			<li><a href="${path}/list/pageNum/${pb.totalPage}?keyword=${board.keyword}&search=${board.search}">
