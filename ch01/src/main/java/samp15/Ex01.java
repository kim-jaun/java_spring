...
public class Ex01 {
	// 전역변수(여러 메서드에서 같이 사용), static 클래스 변수, 객체를 생성하지 않고 바로 사용 가능
	private static MemberService ms = null;
	public static void main(String[] args) {
		AbstractApplicationContext ac = 
			new ClassPathXmlApplicationContext("/samp15/beans15.xml");
		...
		while(true) {
			...
			if (command.equals("x")) break;
				...
				// 입력된 명령어를 컴마(,)를 기준으로 나누어서 배열로 만들어라
	...
	private static void update(String[] str) {
		...
		if (!str[2].equals(str[3])) {
			System.out.println("암호와 암호 학인이 다릅니다");
			return;
		}
	...
	private static void help() {
		...
		System.out.println("다음 명령어 방식을 사용하세요");
